...
import { type Feed, norm, fmtShort } from "../lib/feed";
...
export default function RankingView({ feed }: { feed: Feed }) {
  const ranking = useMemo(() => buildRanking(feed.setlists ?? []), [feed]);
    ...
    <main className="mx-auto w-full min-h-dvh max-w-xl px-5 pb-24 pt-4">
      <header className="mb-5">
        ...
        <p className="mt-2 break-keep text-base leading-relaxed text-stone-600">
          많이 부른 찬양 순서예요. 총{" "}
          <b className="font-semibold text-stone-800">{ranking.length}곡</b>을
          불렀어요.
        </p>
      </header>
